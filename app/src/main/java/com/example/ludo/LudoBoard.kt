...
class LudoBoard(context: Context?, attributeSet: AttributeSet?) : View(context, attributeSet) {
    ...
    private fun movePiece(canvas: Canvas) {
        ...
        paint.style = Style.STROKE
        var bitmap = BitmapFactory.decodeResource(resources, R.drawable.blue)
        val radius = p * 0.48f
        ...
        val board = Board()
        board.setPathMap()
        val list = mutableListOf<com.example.ludo.Pair>()
        for(i in 0..17){
            val pair = board.getPathMap(i)
            val x = p*pair.x
            val y = shift + p*pair.y
            GlobalScope.launch(Dispatchers.Main){
                delayedDisplay(canvas!!, bitmap, x, y)
            }
        }
    }
    private suspend fun delayedDisplay(canvas: Canvas, bitmap: Bitmap, x: Float, y: Float){
